...
class CategoryService:
    ...
    async def get_all_categories(self, skip: int = 0, limit: int =100) -> list[Category]:
        ...
        return [Category.model_validate(category) for category in db_categories]
    ...
    async def create_category(self, category: CategoryBase) -> Category | None:
        ...
        if existing_category:
            return None
        db_category = await self.category_repo.create(name=category.name)# Возвращаем None, если категория с таким именем уже есть
        return Category.model_validate(db_category)
